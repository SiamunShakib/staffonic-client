...
const AllEmployeeList = () => {
    ...
    fetch("http://localhost:5000/users")
      ...
      .then((data) => {
        ...
        const employeeCount = validUsers.filter(
          (u) => u.role === "employee" && !u.fired
        ).length;
        const hrCount = validUsers.filter(
          (u) => u.role === "hr" && !u.fired
        ).length;
        const firedCount = validUsers.filter((u) => u.fired).length;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`http://localhost:5000/users/${emp._id}/fire`, {
          ...
          .then(() => {
            setEmployees((prev) =>
              prev.map((e) => (e._id === emp._id ? { ...e, fired: true } : e))
            );
  ...
  const makeHR = (emp) => {
    fetch(`http://localhost:5000/users/${emp._id}/makeHR`, {
      ...
      .then(() => {
        setEmployees((prev) =>
          prev.map((e) => (e._id === emp._id ? { ...e, role: "hr" } : e))
        );
  ...
  const saveSalary = (empId) => {
    ...
    fetch(`http://localhost:5000/users/${empId}/salary`, {
      ...
      .then(() => {
        setEmployees((prev) =>
          prev.map((e) =>
            e._id === empId ? { ...e, salary: parsedSalary } : e
          )
        );
      ...
      <div className="max-w-7xl px-3 md:px-5 mx-auto">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-600">
            Admin Dashboard - Manage all employees and HR staff
          </p>
        </div>
